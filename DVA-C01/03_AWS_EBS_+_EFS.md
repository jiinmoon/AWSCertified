...
- EBS volumes can be chosen when we are creating the EC2 instance.
- The root volume has to be either GP2 or IOI.
...
- However, after attaching the EBS volume, we need to format the block device
    with a file system, and then mount it.
- To do so, we SSH into the running EC2 instance, make a filesystem, and mount
    the EBS volume.
...
- New volumes are raw block devices, hence we need to create a file system.
...
- Very High IOPS, reaching upto million.
- Disks up to 7.5 TB, stripped to reach 30 TB.
- Block Strage (just like EBS).
...
- Highly available, scalable, expensive (3 times the gp2); but pay per use.
...
- Pay per usage.
- About 3 times more expensive as EBS.
